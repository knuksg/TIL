...
```python
values = Director.objects.all()
for value in values:
		print(value.name, value.debut, value.country)
```
...
```python
value = Director.objects.get(id=1)
print(value.name, value.debut, value.country)
```
...
```
country 컬럼 안에 USA 값을 지닌 레코드가 없기 때문입니다.
```
...
```python
values = Director.objects.filter(country='KOR')
for value in values:
    print(value.name, value.debut, value.country)
```
...
```
get은 값이 없거나 값이 2개 이상일 때 오류가 발생합니다.
get의 결과값 타입은 db.models.Director이고,
filter의 결과값 타입은 django.db.models.query.QuerySet입니다.
```
